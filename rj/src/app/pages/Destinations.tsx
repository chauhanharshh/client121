            ...
            >
              ...
              <div className="p-6 border-t border-[var(--color-text-primary)]/10 bg-[var(--color-bg-light)] flex justify-between items-center">
                ...
                <div className="flex gap-4 w-full sm:w-auto">
                  <Link
                    to="/contact"
                    className="flex-1 sm:flex-none bg-[var(--color-text-primary)] text-white px-8 py-3.5 text-[11px] uppercase tracking-[0.2em] hover:bg-[var(--color-accent-primary)] hover:text-[var(--color-text-primary)] transition-colors text-center font-medium"
                    style={{ fontFamily: "'Inter', sans-serif" }}
                  >
                    Enquire Now
                  </Link>
...
export default function Destinations() {
  ...
  const filtered = allDestinations.filter((d) => {
    ...
    if (activeCategory === "all") {
      ...
      matchesCategory = d.tag.toLowerCase().includes("adventure") || d.description.toLowerCase().includes("adventure");
    } else if (activeCategory === "pilgrimage" || activeCategory === "religious") {
      matchesCategory = d.tag.toLowerCase().includes("spiritual") || d.description.toLowerCase().includes("temple") || d.description.toLowerCase().includes("sacred");
    } else if (activeCategory === "treks") {
      matchesCategory = d.tag.toLowerCase().includes("mountains") || d.description.toLowerCase().includes("trek") || d.tag.toLowerCase().includes("adventure");
    } else if (activeCategory === "family") {
      matchesCategory = d.tag.toLowerCase().includes("heritage") || d.tag.toLowerCase().includes("coastal") || d.tag.toLowerCase().includes("nature");
    }
      ...
      <div className="bg-[var(--color-bg)] min-h-screen">
        ...
        <section className="px-6 lg:px-10 pb-32">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="sticky top-[73px] z-40 bg-[var(--color-bg)] pt-4 pb-4 border-b border-black/5 mb-12 -mx-6 px-6 lg:mx-0 lg:px-0">
              <div className="flex flex-col md:flex-row gap-4 md:items-center max-w-7xl mx-auto">
                ...
                <div className="relative w-full md:w-64 flex-none">
                  <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-black/40" size={14} />
                  <input
                    type="text"
                    placeholder="Search destinations..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="w-full pl-10 pr-5 py-3 rounded-full text-[12px] bg-[#fafafa] border border-black/10 focus:border-black/30 outline-none font-medium transition-colors duration-300 placeholder:text-black/30"
                    style={{ fontFamily: "'Inter', sans-serif" }}
                  />
                </div>

                <div className="hidden md:block w-px h-6 bg-black/10 flex-none mx-2" />
        ...
        {/* Bottom CTA */}
        <section className="bg-[#e2e4e6] py-24 px-6 text-center">
          <motion.div
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
